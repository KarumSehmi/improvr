...
const DEFS: Def[] = [
  ...
  { id: 'teeth-30', emoji: '🪥', title: 'Dentist Approved', detail: 'Brush your teeth every night for 30 days', target: 30, value: best('teethPm') },
  { id: 'checkin-3', emoji: '🎯', title: 'Hat-trick', detail: 'Check in morning, afternoon and evening in one day', target: 1, value: (s) => s.evals.filter((e) => e.checkins >= 3).length },
  { id: 'checkin-21', emoji: '📡', title: 'Always On', detail: '21 hat-trick days', target: 21, value: (s) => s.evals.filter((e) => e.checkins >= 3).length },
  { id: 'quest-10', emoji: '🎲', title: 'Side Quester', detail: 'Complete 10 bonus quests', target: 10, value: (s) => s.evals.filter((e) => e.log?.quest?.done).length },
  { id: 'chest-legend', emoji: '👑', title: 'Jackpot', detail: 'Pull a legendary reward chest', target: 1, value: (s) => s.evals.filter((e) => (e.log?.chest ?? 0) >= 75).length },
